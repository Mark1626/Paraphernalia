...
impl GossipBridge {
    ...
    ///
    /// When `peers` is empty (first node), uses `subscribe` which returns immediately.
    /// When peers are provided, uses `subscribe_and_join` which waits for connectivity.
    pub async fn join(
        &self,
        ...
        Ok((
            SignalSender { inner: sender },
            SignalReceiver { inner: receiver },
        ))
    }
}
