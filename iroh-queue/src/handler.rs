...
use crate::protocol::{self, QueueRequest, QueueResponse};
use crate::queue::JobStore;

/// Protocol handler for the iroh-queue direct stream protocol.
///
...
impl ProtocolHandler for QueueProtocol {
    /// Accept an incoming connection and handle multiple bi-directional streams.
    /// Each stream handles one request-response cycle.
    async fn accept(&self, connection: Connection) -> Result<(), AcceptError> {
        let remote_id = connection.remote_id();
        tracing::debug!(%remote_id, "accepted queue connection");

        // Loop accepting bi-directional streams on this connection.
        // Each stream is a single request-response exchange.
        loop {
...
/// Handle a single bi-directional stream: read request, process, write response.
///
/// The protocol supports two patterns on a single stream:
/// 1. Claim flow: Claim -> Granted, then Ack/Nack -> Acked/Error
/// 2. Standalone: single request -> single response
async fn handle_stream(
    ...
    match request {
        QueueRequest::Claim { job_id } => {
            ...
            match store.try_claim(job_id, remote_id).await {
                Ok(job) => {
                    ...
                    .await
                    {
                        Ok(Ok(QueueRequest::Ack { job_id, .. })) => {
                            store.ack(job_id).await?;
                            protocol::write_message(
                                &mut send,
                        ...
                        Ok(Err(e)) => {
                            ...
                            // Job will be reaped by the stale reaper
                        }
                        Err(_) => {
                ...
                Err(crate::QueueError::AlreadyClaimed(_)) => {
                    protocol::write_message(
                        &mut send,
        ...
        // Standalone ack (if consumer reconnects after disconnect)
    ...
    send.finish().map_err(|e| crate::QueueError::Connection(e.to_string()))?;
    Ok(())
}
